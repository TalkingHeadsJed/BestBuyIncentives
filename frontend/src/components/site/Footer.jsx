    ...
    <footer data-testid="site-footer" className="bg-[#0A0A0A] text-white">
      <div className="border-b border-white/10">
        <div className="mx-auto max-w-[1400px] px-6 lg:px-10 py-16 lg:py-20 grid lg:grid-cols-2 gap-12 items-center">
          ...
          <div className="flex flex-col items-start lg:items-end gap-4">
            <Link to="/contact" data-testid="footer-cta-demo" className="inline-flex items-center gap-3 bg-[#FFD300] hover:bg-[#FFEA66] text-black font-bold text-lg px-8 py-5 uppercase tracking-wide">Book My Strategy Call <ArrowRight className="h-5 w-5" /></Link>
      ...
      <div className="mx-auto max-w-[1400px] px-6 lg:px-10 py-16 grid grid-cols-1 md:grid-cols-12 gap-10">
        <div className="md:col-span-5">
          <Link to="/" className="flex items-center gap-3"><span className="inline-block h-9 w-9 bg-[#FFD300] flex items-center justify-center font-display font-bold text-base text-black">BB</span><span className="font-display font-bold text-xl">BestBuy<span className="text-[#FFD300]">Incentives</span></span></Link>
        ...
        <FooterColumn title="Sales system" links={[["How It Works", "/how-it-works/"], ["Sales Closing Incentives", "/sales-closing-incentives/"], ["Customer Incentive Programs", "/customer-incentive-programs/"], ["Compare Incentives", "/customer-incentive-comparisons/"], ["Book a Call", "/contact"]]} />
        <FooterColumn title="Company" links={[["About", "/about"], ["Case Studies", "/case-studies"], ["Resources", "/resources"], ["Industries", "/industries"], ["FAQ", "/faq"]]} />
      ...
      <div className="border-t border-white/10">
        <div className="mx-auto max-w-[1400px] px-6 lg:px-10 py-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-6 text-[11px] font-mono uppercase tracking-widest text-white/40"><Link to="/privacy" className="hover:text-[#FFD300]">Privacy</Link><Link to="/terms" className="hover:text-[#FFD300]">Terms</Link><Link to="/compliance" className="hover:text-[#FFD300]">Compliance</Link></div>
        </div>
      </div>
    </footer>
  );
...
function FooterColumn({ title, links }) {
  return <div className="md:col-span-2"><div className="text-[11px] font-mono uppercase tracking-widest text-white/40 mb-4">{title}</div><ul className="space-y-2.5 text-sm">{links.map(([label, to]) => <li key={to}><Link to={to} className="text-white/80 hover:text-[#FFD300]">{label}</Link></li>)}</ul></div>;
}
